...
pub fn check_claude_md(path: &Path, display_name: &str) -> Vec<Issue> {
    ...
    if tokens > 5000 {
        ...
        issues.push(
            Issue::warning(
                display_name,
                &format!("Large CLAUDE.md (~{} tokens, {} lines)", tokens, lines),
            )
    ...
    if lower.contains("always add comments") && lower.contains("minimal comments") {
        issues.push(
            Issue::error(
                display_name,
                "Contradictory rules: 'always add comments' vs 'minimal comments'",
            )
    ...
    if lower.contains("always add jsdoc") && lower.contains("no jsdoc") {
        issues.push(
            Issue::error(
                display_name,
                "Contradictory rules: 'always add JSDoc' vs 'no JSDoc'",
            )
            .with_suggestion("Pick one approach and remove the other"),
        );
    }
    ...
    if long_lines > 0 {
        issues.push(
            Issue::warning(
                display_name,
                &format!(
                    "{} lines over 500 chars (likely copy-pasted content)",
                    long_lines
                ),
            )
    ...
    let vague_patterns = [
        (
            "write clean code",
            "Too vague. Specify what 'clean' means (naming conventions, max function length, etc.)",
        ),
        (
            "follow best practices",
            "Too vague. Which practices? Be specific or remove.",
        ),
        (
            "be helpful",
            "Claude already tries to be helpful. This wastes tokens.",
        ),
        (
            "you are an expert",
            "Role-playing prompts are unnecessary in CLAUDE.md. Claude Code already knows it's a coding assistant.",
        ),
        (
            "you are a senior",
            "Role-playing prompts waste tokens. Remove.",
        ),
    ];
    ...
    if content
        .chars()
        .any(|c| ('\u{1F600}'..='\u{1F9FF}').contains(&c))
    {
        issues.push(
    ...
    if headers.is_empty() && lines > 10 {
        issues.push(
            Issue::warning(
                display_name,
                "No markdown headers found in a file with {} lines",
            )
            .with_suggestion(
                "Add headers (## Section) to help Claude parse your rules efficiently",
            ),
        );
    }
...
pub fn check_settings(path: &Path) -> Vec<Issue> {
        ...
        if hook_count > 10 {
            issues.push(
                Issue::warning(
                    "settings.json",
                    &format!(
                        "{} hooks configured. Each hook adds latency to every tool call.",
                        hook_count
                    ),
                )
                ...
                for handler in arr {
                    if handler.get("matcher").is_none() && event == "PreToolUse" {
                        issues.push(
                            Issue::warning(
                                "settings.json",
                                "PreToolUse hook without matcher runs on EVERY tool call",
                            )
                            .with_suggestion(
                                "Add a matcher to limit which tools trigger this hook",
                            ),
                        );
                    }
                }
    ...
    if json.get("skipDangerousModePermissionPrompt") == Some(&serde_json::Value::Bool(true)) {
        issues.push(
            Issue::info(
                "settings.json",
                "skipDangerousModePermissionPrompt is enabled",
            )
            .with_suggestion(
                "This skips safety confirmations. Make sure you understand the risks.",
            ),
        );
    }
...
pub fn check_skills(dir: &Path) -> Vec<Issue> {
    ...
    let skill_count = entries.iter().filter(|e| e.path().is_dir()).count();
    ...
    if skill_count > 20 {
        issues.push(
            Issue::warning(
                "~/.claude/skills/",
                &format!(
                    "{} skills installed. Each skill's description is loaded into context.",
                    skill_count
                ),
            )
            .with_suggestion(
                "Remove skills you don't use. Each unused skill wastes ~50-200 tokens per session.",
            )
            .with_impact(&format!(
                "~{}-{} tokens from skill descriptions alone",
                skill_count * 50,
                skill_count * 200
            )),
        );
    ...
    if cmd_count > 30 {
        issues.push(
            Issue::warning(
                "~/.claude/commands/",
                &format!(
                    "{} commands configured. Commands are listed in context when relevant.",
                    cmd_count
                ),
            )
